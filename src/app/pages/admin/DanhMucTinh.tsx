...
const initialItems: TinhItem[] = [
  { id: "1", code: "T01", name: "Hà Nội", description: "Thủ đô Hà Nội" },
  { id: "2", code: "T02", name: "Hồ Chí Minh", description: "Thành phố Hồ Chí Minh" },
  { id: "3", code: "T03", name: "Đà Nẵng", description: "Thành phố Đà Nẵng" },
];

export default function DanhMucTinh() {
  const [items, setItems] = useState<TinhItem[]>(initialItems);
  const [searchTerm, setSearchTerm] = useState("");
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<TinhItem | null>(null);

  const [formData, setFormData] = useState({ code: "", name: "", description: "" });

  const handleAdd = () => {
    if (!formData.name || !formData.code) {
      toast.error("Vui lòng nhập đủ Mã và Tên tỉnh!");
      return;
    }
    const newItem: TinhItem = {
  ...
  const openEditDialog = (item: TinhItem) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Quản lý danh mục địa phận tỉnh</h1>
        </div>
      ...
      <Card className="p-4 border-slate-200/60 shadow-sm bg-white/50 backdrop-blur-sm">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <Input 
              placeholder="Tìm kiếm danh mục tỉnh..." 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="py-4 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider w-32">Mã tỉnh</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider">Tên tỉnh</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider">Mô tả</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider text-right w-28">Thao tác</th>
              </tr>
            </thead>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <label className="text-sm font-medium">Mã tỉnh <span className="text-red-500">*</span></label>
              <Input value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} placeholder="Ví dụ: T01" />
            </div>
            <div className="grid gap-2">
              <label className="text-sm font-medium">Tên tỉnh <span className="text-red-500">*</span></label>
              <Input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Nhập tên tỉnh..." />
            </div>
            <div className="grid gap-2">
