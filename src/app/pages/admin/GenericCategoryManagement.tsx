...
interface GenericCategoryManagementProps {
  title: string;
  itemLabel?: string;
  initialData?: CategoryItem[];
}

export default function GenericCategoryManagement({ title, itemLabel = "danh mục", initialData }: GenericCategoryManagementProps) {
  const [items, setItems] = useState<CategoryItem[]>(initialData || [
    { id: "1", code: "CODE_01", name: `Mục mẫu 1`, description: "Mô tả mẫu 1" },
    { id: "2", code: "CODE_02", name: `Mục mẫu 2`, description: "Mô tả mẫu 2" },
  ]);
  ...
  const handleAdd = () => {
    if (!formData.name || !formData.code) {
      toast.error(`Vui lòng nhập đủ Mã và Tên ${itemLabel}!`);
      return;
    }
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="py-4 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider w-32">Mã {itemLabel}</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-600 uppercase tracking-wider">Tên {itemLabel}</th>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <label className="text-sm font-medium">Mã {itemLabel} <span className="text-red-500">*</span></label>
              <Input value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} placeholder={`Nhập mã ${itemLabel}...`} />
            </div>
            <div className="grid gap-2">
              <label className="text-sm font-medium">Tên {itemLabel} <span className="text-red-500">*</span></label>
              <Input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder={`Nhập tên ${itemLabel}...`} />
            </div>
            <div className="grid gap-2">
