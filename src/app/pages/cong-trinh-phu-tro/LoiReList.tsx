...
const mockData: any[] = [
  { 
    ...
    gocNghieng: "45",
    viTri: "Bên phải", 
    toChucGT: "Vạch sơn, biển báo",
    trangThai: "Hoạt động",
    ngayApDung: "01/01/2024",
    ghiChu: "Lối rẽ vào khu dân cư, tầm nhìn tốt."
  },
];

export default function LoiReList() {
  const [viewMode, setViewMode] = useState<"list" | "map">("list");
  const [data, setData] = useState<LoiRe[]>(mockData);
  const [searchTerm, setSearchTerm] = useState("");
  const [isDetailDialogOpen, setIsDetailDialogOpen] = useState(false);
  const [isImportDialogOpen, setIsImportDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<LoiRe | null>(null);
  ...
  const loiReFields = [
    ...
    { name: "gocNghieng", label: "Góc nghiêng (độ)" },
    { name: "viTri", label: "Vị trí" },
    { name: "toChucGiaoThong", label: "Tổ chức giao thông" },
  ...
  return (
    <div className="space-y-6">
      ...
      {viewMode === "list" && (
        <Card>
        <CardHeader>
          <CardTitle>Danh sách Lối rẽ ({filteredData.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="rounded-md border overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Tên lối rẽ</TableHead>
              ...
              <TableBody>
                {filteredData.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell className="font-medium">{item.ten}</TableCell>
      ...
      <DetailDialog
        ...
        selectedItem={selectedItem ? { ...selectedItem, fullName: selectedItem.ten, idNumber: selectedItem.id, status: selectedItem.trangThai } : null}
        selectedCard={{ title: "Lối rẽ" }}
        onEditClick={() => {}}
      />
