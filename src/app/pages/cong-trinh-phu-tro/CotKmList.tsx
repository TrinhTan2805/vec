...
import { Search, Plus, Edit, Trash2, Download, Eye, Upload, LayoutDashboard, Map as MapIcon, Layers, MapPin, BarChart3, AlertTriangle, ShieldCheck } from "lucide-react";
...
const mockData: any[] = [
  { 
    ...
    ghiChu: "Vị trí khởi đầu tuyến đường."
  },
  { 
    id: "KM-002", 
    tuyenDuong: "QL1A", 
    ...
    trangThai: "Hoạt động",
    ngayApDung: "01/01/2024",
    ghiChu: "Tình trạng ổn định."
  },
];

export default function CotKmList() {
  const [viewMode, setViewMode] = useState<"list" | "map">("list");
  const [data, setData] = useState<CotKm[]>(mockData);
  const [searchTerm, setSearchTerm] = useState("");
  const [isDetailDialogOpen, setIsDetailDialogOpen] = useState(false);
  const [isImportDialogOpen, setIsImportDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<CotKm | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {viewMode === "list" && (
        <Card>
        <CardHeader>
          <CardTitle>Danh sách Cột Km ({filteredData.length})</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <DetailDialog
        ...
        selectedItem={selectedItem ? { ...selectedItem, fullName: `Cột Km ${selectedItem.lyTrinh}`, idNumber: selectedItem.id, status: selectedItem.trangThai } : null}
        selectedCard={{ title: "Cột Km" }}
        onEditClick={() => {}}
      />
