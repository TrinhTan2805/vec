...
const mockData: any[] = [
  { 
    ...
    chieuRong: "0.6",
    viTri: "Bên phải", 
    tenLoaiRanh: "Rãnh dọc thoát nước",
    ketCauRanh: "Bê tông cốt thép",
    trangThai: "Tốt",
    ngayApDung: "01/01/2024",
    ghiChu: "Rãnh thoát nước dọc tuyến, không bị tắc nghẽn."
  },
];

export default function RanhNuocList() {
  ...
  const fields = [
    ...
    { name: "tenLoaiRanh", label: "Tên loại rãnh" },
    { name: "ketCauRanh", label: "Kết cấu rãnh" },
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-1 items-center gap-2">
              ...
              <Input
                placeholder="Tìm kiếm rãnh nước..."
                value={searchTerm}
      ...
      {viewMode === "list" && (
        <Card>
        <CardHeader>
          <CardTitle>Danh sách Rãnh nước ({filteredData.length})</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {filteredData.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.chieuDai} m</TableCell>
                    <TableCell>{item.ketCauRanh}</TableCell>
                    <TableCell>{item.viTri}</TableCell>
                    <TableCell>
                      <Badge variant={item.trangThai === "Tốt" ? "default" : "secondary"}>
                        {item.trangThai}
                      </Badge>
                    </TableCell>
      ...
      <DetailDialog
        ...
        selectedItem={selectedItem ? { ...selectedItem, fullName: `Rãnh nước ${selectedItem.lyTrinh}`, idNumber: selectedItem.id, status: selectedItem.trangThai } : null}
        selectedCard={{ title: "Rãnh nước" }}
        onEditClick={() => {}}
      />
